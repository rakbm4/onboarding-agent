...
def cmd_check_config():
    ...
    checks = [
        ("SLACK_BOT_TOKEN",    True,  "starts with xoxb-"),
        ("SLACK_LOG_CHANNEL",  True,  "starts with C (channel ID)"),
        ("GOOGLE_SHEET_ID",    True,  "44-char alphanumeric string"),
        ("JOINERS_SHEET_GID",  False, "numeric string"),
        ("HOLIDAYS_SHEET_GID", False, "numeric string"),
        ("DOCS_SHEET_GID",     False, "numeric string"),
    ]
    all_ok = True
    for name, required, fmt in checks:
        val = os.environ.get(name, "")
        if val:
            display = val[:10] + "..." if len(val) > 10 else val
            warn = (name == "SLACK_BOT_TOKEN" and not val.startswith("xoxb-")) or \
                   (name == "SLACK_LOG_CHANNEL" and not val.startswith("C"))
            prefix = "⚠️ " if warn else "✅"
            print(f"  {prefix} {name} = {display}")
        elif required:
            print(f"  ❌ {name} = MISSING  (required — {fmt})")
            all_ok = False
        else:
            print(f"  ⚙️  {name} = not set  (optional — {fmt})")
    ...
    if token:
        ...
        try:
            ...
            result = r.json()
            if result.get("ok"):
                print(f"  ✅ Slack OK — bot: {result.get('bot_id')}, workspace: {result.get('team')}")
            else:
                print(f"  ❌ Slack auth failed: {result.get('error')}")
                all_ok = False
        except Exception as e:
    ...
    if sheet_id:
        ...
        try:
            rows = fetch_sheet_tab(sheet_id, os.environ.get("JOINERS_SHEET_GID", "0"))
            print(f"  ✅ Sheet accessible — {len(rows)} joiner rows found")
        except Exception as e:
            print(f"  ❌ Sheet fetch failed: {e}")
            all_ok = False

    print(f"\n{'✅ PASS' if all_ok else '❌ FAIL'} — configuration check complete\n")


def cmd_check_sheets():
    print("\n=== Google Sheets Data ===\n")
    sheet_id = os.environ.get("GOOGLE_SHEET_ID", "")
    if not sheet_id:
    ...
    try:
        rows = fetch_sheet_tab(sheet_id, os.environ.get("JOINERS_SHEET_GID", "0"))
        print(f"  {'Name':<25} {'Email':<30} {'SlackID':<15} {'Joining':<12} Status")
        print("  " + "-" * 90)
        for r in rows:
            name    = r.get("name", "")
            email   = r.get("email", "")
            slack   = r.get("slackid", "") or r.get("slackuserid", "")
            joining = r.get("joiningdate", "") or r.get("joindate", "")
            issues  = []
            if not name:    issues.append("missing name")
            if not joining: issues.append("missing date")
            else:
                try:
                    d = date.fromisoformat(joining.replace("/", "-"))
                    if d.weekday() not in (0, 2):
                        issues.append(f"not Mon/Wed ({d.strftime('%A')})")
                except ValueError:
                    issues.append(f"bad date: {joining}")
            if not slack and not email:
                issues.append("no SlackID or email")
            warnings += len(issues)
            status = ("⚠️  " + ", ".join(issues)) if issues else "✅"
            print(f"  {name:<25} {email:<30} {slack:<15} {joining:<12} {status}")
        print(f"\n  Total: {len(rows)} joiners\n")
    except Exception as e:
        print(f"  ❌ Failed: {e}\n")

    print("── Tab 2: Holidays ──")
    try:
        rows = fetch_sheet_tab(sheet_id, os.environ.get("HOLIDAYS_SHEET_GID", "713153074"))
        print(f"  {'Date':<15} {'Holiday Name':<30} Status")
        print("  " + "-" * 55)
        for r in rows:
            d    = r.get("date", "") or r.get("holidaydate", "")
            name = r.get("holidayname", "") or r.get("name", "")
            try:
                date.fromisoformat(d.replace("/", "-"))
                status = "✅"
            except ValueError:
                status = "⚠️  bad date format"
                warnings += 1
            print(f"  {d:<15} {name:<30} {status}")
        print(f"\n  Total: {len(rows)} holidays\n")
    except Exception as e:
        print(f"  ❌ Failed: {e}\n")

    print("── Tab 3: Documents ──")
    gid = os.environ.get("DOCS_SHEET_GID", "")
    if not gid:
        print("  ⚙️  DOCS_SHEET_GID not set — skipping\n")
    else:
        try:
            rows = fetch_sheet_tab(sheet_id, gid)
            print(f"  {'Day':<5} {'Document Name':<30} {'Link':<45} Status")
            print("  " + "-" * 85)
            for r in rows:
                day  = r.get("day", "")
                name = r.get("documentname", "") or r.get("docname", "") or r.get("name", "")
                link = r.get("link", "") or r.get("url", "") or r.get("documentlink", "")
                issues = []
                if not (day.isdigit() and 1 <= int(day) <= 5): issues.append("day not 1–5")
                if not link: issues.append("no link")
                warnings += len(issues)
                status       = ("⚠️  " + ", ".join(issues)) if issues else "✅"
                link_display = (link[:42] + "...") if len(link) > 45 else link
                print(f"  {day:<5} {name:<30} {link_display:<45} {status}")
            print(f"\n  Total: {len(rows)} document rows\n")
        except Exception as e:
            print(f"  ❌ Failed: {e}\n")

    holidays = load_company_holidays()
    today    = date.today()
    working  = is_working_day(today, holidays)
    print(f"Today ({today}) is {'a working day ✅' if working else 'NOT a working day ⏸'}")
    print(f"Validation warnings: {warnings}\n")


def cmd_check_schedule(joining_date_str: str):
    print(f"\n=== Onboarding Schedule for {joining_date_str} ===\n")
    try:
        joining = date.fromisoformat(joining_date_str.replace("/", "-"))
    except ValueError:
        print(f"❌ Invalid date '{joining_date_str}'. Use YYYY-MM-DD.\n")
        return

    if joining.weekday() not in (0, 2):
        day_name = joining.strftime("%A")
        candidates = [joining + timedelta(days=(wd - joining.weekday()) % 7) for wd in (0, 2)]
        candidates = [c for c in candidates if c != joining]
        nearest = min(candidates)
        print(f"❌ {joining_date_str} is a {day_name}. Joining must be Monday or Wednesday.")
        print(f"   Nearest valid date: {nearest.isoformat()}\n")
        return

    holidays = load_company_holidays()
    today    = date.today()
    print(f"  {'Day':<5} {'Date':<12} {'Weekday':<12} Notes")
    print("  " + "-" * 50)
    ...
    while day_num < 5:
        if is_working_day(current, holidays):
            day_num += 1
            if current < today:       note = "past"
            elif current == today:    note = "← today"
            else:                     note = "upcoming"
            if day_num == 1:          note += " (joining day)"
            print(f"  {day_num:<5} {current.isoformat():<12} {current.strftime('%A'):<12} {note}")
        elif current.weekday() < 5:
            print(f"  {'─':<5} {current.isoformat():<12} {current.strftime('%A'):<12} holiday — skipped")
        current += timedelta(days=1)
    print()


def cmd_preview_message(day: int, slack_id: str = "UPREVIEW"):
    if day not in range(1, 6):
        print(f"❌ Day must be 1–5, got: {day}\n")
        return
    docs    = load_documents()
    message = compose_message(slack_id, day, docs[day])
    print(f"\n=== Day {day} Message Preview  (slack_id: {slack_id}) ===\n")
    print("─" * 60)
    print(message)
    print("─" * 60)
    if docs[day]:
        print(f"\nDocuments substituted for Day {day}:")
        for name, link in docs[day]:
            print(f"  • {name}: {link or '(pending)'}")
    else:
        print(f"\nNo documents configured for Day {day} — all links show as pending.")
    print()
...
def cmd_dry_run():
    ...
    if not is_working_day(today, holidays):
        print("⏸  Not a working day — agent would skip today\n")
        return

    print("✅ Working day — agent would run\n")
    joiners = load_joiners()
    docs    = load_documents()
    print(f"Joiners loaded: {len(joiners)}\n")
    ...
    for j in joiners:
        try:
            ...
            print(f"❌  {j['name']} — bad date format: '{j['joining']}'")
            continue

        day_num = get_onboarding_day(joining_date, today, holidays)
        if not day_num:
            print(f"⏭   {j['name']} — no message today")
            continue

        user_id = j["slack_id"] if j["slack_id"].startswith("U") else (j["email"] or "UEMAIL")
        message = compose_message(user_id, day_num, docs[day_num])
        print(f"📨  {j['name']} — Day {day_num}")
        print("    Message preview:")
        print("    " + "─" * 56)
        for line in message.split("\n"):
            print(f"    {line}")
        print("    " + "─" * 56 + "\n")
        sent_preview.append(f"✅ {j['name']} — Day {day_num}")

    print("=== SUMMARY (would post to log channel) ===")
    print("\n".join(sent_preview) if sent_preview else "_No onboarding messages scheduled for today._")
    print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dry-run",        action="store_true", help="Simulate today's run without sending Slack messages")
    parser.add_argument("--check-config",   action="store_true", help="Validate env vars and test Slack/Sheets connectivity")
    parser.add_argument("--check-sheets",   action="store_true", help="Display all Google Sheet data with validation warnings")
    parser.add_argument("--check-schedule", metavar="DATE",      help="Show 5-day schedule for a joining date (YYYY-MM-DD)")
    parser.add_argument("--preview-message",metavar="DAY", type=int, help="Preview rendered message for day 1–5")
    parser.add_argument("--slack-id",       metavar="SLACK_ID",  default="UPREVIEW", help="Slack ID for --preview-message (default: UPREVIEW)")
    args = parser.parse_args()

    if   args.dry_run:         cmd_dry_run()
    elif args.check_config:    cmd_check_config()
    elif args.check_sheets:    cmd_check_sheets()
    elif args.check_schedule:  cmd_check_schedule(args.check_schedule)
    elif args.preview_message: cmd_preview_message(args.preview_message, args.slack_id)
    else:                      main()
